<?php
    ...
    function cadastrarLivro($nome, $autor, $isbn, $ano_publicacao){
        global $db;

        $count = 0;
        $query = "INSERT INTO livro (nome, fk_autor, isbn, ano_publicacao, status_ativo)
                VALUES (:nome, :fk_autor, :isbn, :ano_publicacao, 1)";
    ...
    function pesquisarLivro($pesquisa){
        ...
        $query = "SELECT * FROM livro WHERE nome LIKE :pesquisa";
        $statement = $db->prepare($query);
    ...
    function totalAutores(){
        ...
        $count = 0;
        $query = "SELECT l.fk_autor, COUNT(l.fk_autor) FROM livro as l join autor as a group by l.fk_autor";
        $statement = $db->prepare($query);
        if ($statement->execute()) {
            $count = $statement->fetchColumn();
        };
    ...
    //mostrar a tabela com  todos os autores com a qunaatidade de livros
        ...
        $query = "SELECT l.fk_autor, l.nome, COUNT(l.fk_autor) FROM livro as l join autor as a group by l.fk_autor";
        $statement = $db->prepare($query);
        if ($statement->execute()) {
            $result = $statement->fetchAll();
        };
